 ...
 * @date:2023/7/26
 * @author:zx
 * 从摄像头/HDMI输入/X11中获取视频数据，并通过rtsp或者rtmp协议转发出去
 * ./remote_stream -n rtsp://xxx/xxx -cn libx264 rtsp
 * ./remote_stream -n rtmp://xxx/xxx -cn libx264 rtmp
 * 
*/
...
AVPixelFormat in_pix_fmt = AV_PIX_FMT_BGR24; // 截屏是AV_PIX_FMT_RGB32 opencv Mat 数据格式 如果是摄像头或者HDMI是AV_PIX_FMT_BGR24
...
int ImageFromOther(COMMAND_PUSH_OBJ &obj){
    ...
    // videoCap.set(CAP_PROP_FPS, 30);//帧率 帧/秒
    videoCap.set(CAP_PROP_FOURCC,VideoWriter::fourcc('M','J','P','G')); // 捕获格式
    videoCap.open(40); // v4l2-ctl --list-device 查看驱动 41 摄像头 40从hdmi输入
    if(!videoCap.isOpened()){
        cout << "camera not open !" << endl;
        return -1;
    }

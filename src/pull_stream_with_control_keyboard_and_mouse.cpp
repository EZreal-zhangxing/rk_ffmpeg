/**
 * 拉流程序
 * 可以从本地或者网络拉取视频通过硬解码后播放
 * 网络播放：./pullVideo -hd -n rtsp://xxxx/xxx 
 * 本地视频：./pullVideo -hd -l <url>
*/
...
void hardwave_init(AVCodecContext *codecCtx,const AVCodec * codec){
    ...
    /**
     * alloc && init 等同于 create 初始化硬解码器
     * /dev/dri/card0 是硬件驱动
    */
    // int ret = av_hwdevice_ctx_create(&bufferRef,deviceType,"/dev/dri/card0",NULL,0);
    // 将硬解码器关联到解码器上
